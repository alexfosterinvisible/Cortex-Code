"""Git operations for CXC composable architecture.
...
def update_pr_body(pr_number_or_url: str, new_body: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    ...
    Args:
        pr_number_or_url: PR number or full URL
        new_body: New markdown body for the PR
        logger: Logger instance
        
    Returns:
        (success, error_message) tuple
    """
    # Extract PR number if URL was passed
    if pr_number_or_url.startswith("http"):
        pr_number = pr_number_or_url.rstrip("/").split("/")[-1]
    else:
...
def get_pr_number_for_branch(branch_name: str) -> Optional[str]:
...
def approve_pr(pr_number: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--approve",
            "--body",
            "CXC Ship workflow approved this PR after validating all state fields.",
        ],
        capture_output=True,
        text=True,
    )
...
def merge_pr(
    ...
    # Add auto-merge body
    merge_cmd.extend(
        ["--body", "Merged by CXC Ship workflow after successful validation."]
    )
...
def finalize_git_operations(
    state: "CxcState", logger: logging.Logger, cwd: Optional[str] = None
) -> None:
    ...
    if pr_url:
        ...
        if issue_number and cxc_id:
            make_issue_comment(issue_number, f"{cxc_id}_ops: ✅ Pull request: {pr_url}")
    else:
        ...
        if pr_url:
            ...
            if issue_number and cxc_id:
                make_issue_comment(
                    issue_number, f"{cxc_id}_ops: ✅ Pull request created: {pr_url}"
                )
        else:
