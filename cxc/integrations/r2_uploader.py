"""Cloudflare R2 uploader for CXC screenshots."""
...
class R2Uploader:
    ...
    def upload_file(self, file_path: str, object_key: Optional[str] = None) -> Optional[str]:
        ...
        if not object_key:
            # Use pattern: cxc/{cxc_id}/review/{filename}
            object_key = f"cxc/review/{Path(file_path).name}"
    ...
    def upload_screenshots(self, screenshots: List[str], cxc_id: str) -> Dict[str, str]:
        """
        ...
        Args:
            ...
            cxc_id: CXC workflow ID for organizing uploads
        ...
        for screenshot_path in screenshots:
            ...
            # Generate object key with CXC ID for organization
            filename = Path(screenshot_path).name
            object_key = f"cxc/{cxc_id}/review/{filename}"
