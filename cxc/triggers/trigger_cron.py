...
from cxc.integrations.github import fetch_open_issues, fetch_issue_comments, get_repo_url, extract_repo_path
...
def trigger_cxc_workflow(issue_number: int) -> bool:
    """Trigger the CXC plan workflow for a specific issue using the CLI."""
    try:
        print(f"INFO: Triggering CXC plan workflow for issue #{issue_number}")
        
        # Use the cxc CLI to run the plan workflow
        cmd = [sys.executable, "-m", "cxc.cli", "plan", str(issue_number)]
...
def main():
    ...
    print("INFO: Starting CXC cron trigger")
