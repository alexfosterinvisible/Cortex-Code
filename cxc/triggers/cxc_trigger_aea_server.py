...
from cxc.core.aea_data_types import (
    AEAAgent,
    AEAMessage,
    AEAPromptRequest,
    AEAPromptResponse,
...
# Database setup
# Use user's home directory by default, or CXC_DATA_DIR if set
DB_PATH = Path(os.getenv("CXC_DATA_DIR", str(Path.home() / ".cxc" / "data"))) / "aea_agents.db"
...
async def prompt_agent(request: AEAPromptRequest):
    ...
    try:
        # TODO: Implement cxc_aea_patch.py and re-enable this
        return AEAPromptResponse(
            success=False,
            agent=agent,
            error="AEA patch execution not yet implemented - cxc_aea_patch.py missing",
        )

        # NOTE: When re-enabling, use os.getcwd() for cwd (not Path(__file__) navigation)
        # patch_script = Path(os.getenv("CXC_DATA_DIR", str(Path.home() / ".cxc"))) / "cxc_aea_patch.py"
