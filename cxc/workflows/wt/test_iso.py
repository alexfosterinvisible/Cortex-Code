...
"""
CXC Test Iso - Cortex Code for agentic testing in isolated worktrees

Usage:
  uv run python -m cxc.workflows.wt.test_iso <issue-number> <cxc-id> [--skip-e2e]
...
from cxc.core.utils import make_cxc_id, setup_logger, parse_json, check_env_vars
...
def run_tests(cxc_id: str, logger: logging.Logger, working_dir: Optional[str] = None) -> AgentPromptResponse:
...
def post_comprehensive_test_summary(
    ...
    make_issue_comment(
        issue_number, format_issue_message(cxc_id, "test_summary", summary)
    )

    logger.info(f"Posted comprehensive test results summary to issue #{issue_number}")


def run_e2e_tests(cxc_id: str, logger: logging.Logger, working_dir: Optional[str] = None) -> AgentPromptResponse:
...
def run_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        test_response = run_tests(cxc_id, logger, worktree_path)
        ...
        resolved, unresolved = resolve_failed_tests(
            failed_tests, cxc_id, issue_number, logger, worktree_path, iteration=attempt
        )
        ...
        if resolved > 0:
            make_issue_comment(
                issue_number,
                format_issue_message(
                    cxc_id, "ops", f"✅ Resolved {resolved}/{failed_count} failed tests"
                ),
            )
...
def run_e2e_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        e2e_response = run_e2e_tests(cxc_id, logger, worktree_path)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m cxc.workflows.wt.test_iso <issue-number> <cxc-id> [--skip-e2e]")
        print("\nError: cxc-id is required to locate the worktree")
    ...
    temp_logger = setup_logger(cxc_id, "cxc_test_iso")
    state = CxcState.load(cxc_id, temp_logger)
    if state:
        # Found existing state - use the issue number from state if available
        issue_number = state.get("issue_number", issue_number)
        post_state_to_issue(issue_number, cxc_id, state.data, "🔍 Found existing state - starting isolated testing")
    else:
        # No existing state found
        logger = setup_logger(cxc_id, "cxc_test_iso")
        logger.error(f"No state found for CXC ID: {cxc_id}")
        logger.error("Run cxc plan or cxc patch first to create the worktree and state")
        print(f"\nError: No state found for CXC ID: {cxc_id}")
        print("Run cxc plan or cxc patch first to create the worktree and state")
        sys.exit(1)
    
    # Track that this CXC workflow has run
    state.append_cxc_id("cxc_test_iso")
    
    # Set up logger with CXC ID from command line
    logger = setup_logger(cxc_id, "cxc_test_iso")
    logger.info(f"CXC Test Iso starting - ID: {cxc_id}, Issue: {issue_number}, Skip E2E: {skip_e2e}")
    ...
    make_issue_comment(
        issue_number, 
        format_issue_message(cxc_id, "ops", f"✅ Starting isolated testing phase\n"
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, AGENT_TESTER, "🧪 Running unit tests in isolated environment...")
    )
    
    # Run tests with resolution and retry logic
    results, passed_count, failed_count, test_response = run_tests_with_resolution(
        cxc_id, issue_number, logger, worktree_path
    )
    ...
    if results:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_TESTER, comment)
        )
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                cxc_id, AGENT_TESTER, "⚠️ No test results found in output"
            ),
        )
    ...
    if not skip_e2e:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_E2E_TESTER, "🌐 Running E2E tests in isolated environment...")
        )
    ...
    post_comprehensive_test_summary(
        issue_number, cxc_id, test_results, e2e_results, logger
    )
    ...
    if not issue_command:
        ...
        if error:
            ...
            state.save("cxc_test_iso")
    
    # Create commit message
    logger.info("Creating test commit")
    commit_msg, error = create_commit(AGENT_TESTER, issue, issue_command, cxc_id, logger, worktree_path)
    
    if error:
        logger.error(f"Error creating commit message: {error}")
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_TESTER, f"❌ Error creating commit message: {error}")
        )
        sys.exit(1)
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_TESTER, f"❌ Error committing test results: {error}")
        )
        sys.exit(1)
    
    logger.info(f"Committed test results: {commit_msg}")
    make_issue_comment(
        issue_number, format_issue_message(cxc_id, AGENT_TESTER, "✅ Test results committed")
    )
    ...
    make_issue_comment(
        issue_number, format_issue_message(cxc_id, "ops", "✅ Isolated testing phase completed")
    )
    ...
    post_state_to_issue(issue_number, cxc_id, state.data, "📋 Final test state")
    ...
    if total_failures > 0:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, "ops", f"❌ Test workflow completed with {total_failures} failures")
        )
        sys.exit(1)
    else:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, "ops", "✅ All tests passed successfully!")
        )
