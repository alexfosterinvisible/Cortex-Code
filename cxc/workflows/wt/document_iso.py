...
"""
CXC Document Iso - Cortex Code for documentation generation in isolated worktrees

Usage:
  uv run python -m cxc.workflows.wt.document_iso <issue-number> <cxc-id>
...
def track_agentic_kpis(
    ...
    try:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, "ops", "📊 Updating agentic KPIs"),
        )
                ...
                try:
                    commit_msg, error = create_commit(
                        ...
                        GitHubIssue(
                            ...
                            body="Tracking CXC performance metrics",
                            state="open",
                    ...
                    if commit_msg and not error:
                        ...
                        make_issue_comment(
                            issue_number,
                            format_issue_message(
                                cxc_id, "kpi_tracker", "✅ Agentic KPIs updated"
                            ),
                        )
                    elif error:
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m cxc.workflows.wt.document_iso <issue-number> <cxc-id>")
    ...
    temp_logger = setup_logger(cxc_id, "cxc_document_iso")
    state = CxcState.load(cxc_id, temp_logger)
    if state:
        # Found existing state - use the issue number from state if available
        issue_number = state.get("issue_number", issue_number)
        post_state_to_issue(issue_number, cxc_id, state.data, "🔍 Found existing state - starting isolated documentation")
    else:
        # No existing state found
        logger = setup_logger(cxc_id, "cxc_document_iso")
        logger.error(f"No state found for CXC ID: {cxc_id}")
        logger.error(
            "Run cxc plan or cxc patch first to create the worktree and state"
        )
    ...
    state.append_cxc_id("cxc_document_iso")

    # Set up logger with CXC ID from command line
    logger = setup_logger(cxc_id, "cxc_document_iso")
    logger.info(f"CXC Document Iso starting - ID: {cxc_id}, Issue: {issue_number}")
    ...
    if not spec_file:
        ...
        make_issue_comment(
            issue_number, format_issue_message(cxc_id, "ops", f"❌ {error_msg}")
        )
        sys.exit(1)
    ...
    doc_result = generate_documentation(
        issue_number, cxc_id, logger, spec_file, working_dir=worktree_path
    )
    ...
    if doc_result.documentation_created:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                cxc_id, AGENT_DOCUMENTER, "ℹ️ No documentation changes were needed"
            ),
        )
    ...
    commit_msg, error = create_commit(
        AGENT_DOCUMENTER, issue, issue_command, cxc_id, logger, worktree_path
    )
    ...
    if error:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                cxc_id, AGENT_DOCUMENTER, f"❌ Error creating commit message: {error}"
            ),
        )
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, AGENT_DOCUMENTER, "✅ Documentation committed"),
    )

    # Track Agentic KPIs before finalizing - this never fails the workflow
    track_agentic_kpis(issue_number, cxc_id, state, logger, worktree_path)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(
            cxc_id, "ops", "✅ Isolated documentation phase completed"
        ),
    )

    # Save final state
    state.save("cxc_document_iso")

    # Post final state summary to issue
    post_state_to_issue(issue_number, cxc_id, state.data, "📋 Final documentation state")
