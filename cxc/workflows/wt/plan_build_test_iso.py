...
"""
CXC Plan Build Test Iso - Compositional workflow for isolated planning, building, and testing

Usage: uv run python -m cxc.workflows.wt.plan_build_test_iso <issue-number> [cxc-id] [--skip-e2e]
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run python -m cxc.workflows.wt.plan_build_test_iso <issue-number> [cxc-id] [--skip-e2e]")
