...
"""
CXC Plan Build Iso - Compositional workflow for isolated planning and building

Usage: uv run python -m cxc.workflows.wt.plan_build_iso <issue-number> [cxc-id]
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: uv run python -m cxc.workflows.wt.plan_build_iso <issue-number> [cxc-id]")
