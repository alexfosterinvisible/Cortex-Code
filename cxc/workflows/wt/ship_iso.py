...
"""
CXC Ship Iso - Cortex Code for shipping (merging) to main

Usage:
  uv run python -m cxc.workflows.wt.ship_iso <issue-number> <cxc-id>
...
every field in CxcState has a value. This is our final approval step.
...
def analyze_merge_conflict(
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(
            cxc_id, AGENT_CONFLICT_ANALYZER,
    ...
    if response.success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_CONFLICT_ANALYZER, msg)
        )
        return None
...
def manual_merge_to_main(branch_name: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "merge", branch_name, "--no-ff", "-m", f"Merge branch '{branch_name}' via CXC Ship workflow"],
            capture_output=True, text=True, cwd=repo_root
        )
        if result.returncode != 0:
            # Try to restore original branch
...
def validate_state_completeness(state: CxcState, logger: logging.Logger) -> tuple[bool, list[str]]:
    """Validate that all fields in CxcState have values (not None).
    
    Returns:
        tuple of (is_valid, missing_fields)
    """
    # Get the expected fields from CxcStateData model
    expected_fields = {
        "cxc_id",
        "issue_number",
        "branch_name",
...
def main():
    ...
    # INTENTIONAL: cxc-id is REQUIRED - we need it to find the worktree and state
    if len(sys.argv) < 3:
        print("Usage: uv run python -m cxc.workflows.wt.ship_iso <issue-number> <cxc-id>")
        print("\nError: Both issue-number and cxc-id are required")
    ...
    temp_logger = setup_logger(cxc_id, "cxc_ship_iso")
    state = CxcState.load(cxc_id, temp_logger)
    if not state:
        # No existing state found
        logger = setup_logger(cxc_id, "cxc_ship_iso")
    ...
    state.append_cxc_id("cxc_ship_iso")
    
    # Set up logger with CXC ID
    logger = setup_logger(cxc_id, "cxc_ship_iso")
    logger.info(f"CXC Ship Iso starting - ID: {cxc_id}, Issue: {issue_number}")
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, "ops", f"🚢 Starting ship workflow\n"
    ...
    if not is_valid:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_SHIPPER, f"❌ {error_msg}\n\n"
                               "Please ensure all workflows have been run:\n"
                               "- cxc plan (creates plan_file, branch_name, issue_class)\n"
                               "- cxc build (implements the plan)\n"
                               "- cxc test (runs tests)\n"
                               "- cxc review (reviews implementation)\n"
                               "- cxc document (generates docs)")
        )
        sys.exit(1)
    ...
    if not valid:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_SHIPPER, f"❌ Worktree validation failed: {error}")
        )
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, AGENT_SHIPPER, f"📋 State validation complete\n"
    ...
    if pr_number:
        ...
        if pr_approved:
            ...
            make_issue_comment(
                issue_number,
                format_issue_message(cxc_id, AGENT_SHIPPER, "✅ PR approved")
            )
        else:
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, AGENT_SHIPPER, f"🔀 Merging {branch_name} to main...\n"
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_SHIPPER, f"❌ Failed to merge: {error}")
        )
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, AGENT_SHIPPER,
    ...
    # Save final state
    state.save("cxc_ship_iso")

    # Post final state summary
    post_state_to_issue(issue_number, cxc_id, state.data, "📋 Final ship state")
