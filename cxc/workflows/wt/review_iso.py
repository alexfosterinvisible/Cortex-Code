...
"""
CXC Review Iso - Cortex Code for agentic review in isolated worktrees

Usage:
  uv run python -m cxc.workflows.wt.review_iso <issue-number> <cxc-id> [--skip-resolution]
...
from cxc.integrations.git_ops import commit_changes, finalize_git_operations, update_pr_body, get_pr_number_for_branch
...
from cxc.core.utils import setup_logger, parse_json, check_env_vars
...
def run_review(
    ...
    request = AgentTemplateRequest(
        ...
        args=[cxc_id, spec_file, AGENT_REVIEWER],
        cxc_id=cxc_id,
...
def upload_review_screenshots(
    ...
    for local_path in review_result.screenshots:
        ...
        remote_path = f"cxc/{cxc_id}/review/{os.path.basename(local_path)}"
...
def resolve_blocker_issues(
    ...
    for i, issue in enumerate(blocker_issues, 1):
        ...
        # Create patch plan
        plan_response = create_review_patch_plan(issue, i, cxc_id, logger, working_dir=worktree_path)
        ...
        impl_response = implement_plan(plan_file, cxc_id, logger, working_dir=worktree_path)
        # </CALLING_AGENT>
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m cxc.workflows.wt.review_iso <issue-number> <cxc-id> [--skip-resolution]")
    ...
    temp_logger = setup_logger(cxc_id, "cxc_review_iso")
    state = CxcState.load(cxc_id, temp_logger)
    if state:
        # Found existing state - use the issue number from state if available
        issue_number = state.get("issue_number", issue_number)
        post_state_to_issue(issue_number, cxc_id, state.data, "🔍 Found existing state - starting isolated review")
    else:
        # No existing state found
        logger = setup_logger(cxc_id, "cxc_review_iso")
    ...
    state.append_cxc_id("cxc_review_iso")
    
    # Set up logger with CXC ID from command line
    logger = setup_logger(cxc_id, "cxc_review_iso")
    logger.info(f"CXC Review Iso starting - ID: {cxc_id}, Issue: {issue_number}, Skip Resolution: {skip_resolution}")
    ...
    make_issue_comment(
        issue_number, 
        format_issue_message(cxc_id, "ops", f"✅ Starting isolated review phase\n"
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(cxc_id, "ops", f"📋 Found spec file: {spec_file}")
    )
    ...
    while review_attempt < MAX_REVIEW_RETRY_ATTEMPTS:
        ...
        review_result = run_review(spec_file, cxc_id, logger, working_dir=worktree_path)
        ...
        resolve_blocker_issues(blocker_issues, issue_number, cxc_id, worktree_path, logger)
    ...
    if review_result:
        ...
        upload_review_screenshots(review_result, cxc_id, worktree_path, logger)
        
        # Build and post the summary comment
        summary = build_review_summary(review_result)
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_REVIEWER, summary)
        )
    ...
    commit_msg, error = create_commit(AGENT_REVIEWER, issue, issue_command, cxc_id, logger, worktree_path)
    
    if error:
        logger.error(f"Error creating commit message: {error}")
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_REVIEWER, f"❌ Error creating commit message: {error}")
        )
        sys.exit(1)
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(cxc_id, AGENT_REVIEWER, f"❌ Error committing review: {error}")
        )
        sys.exit(1)
    
    logger.info(f"Committed review: {commit_msg}")
    make_issue_comment(
        issue_number, format_issue_message(cxc_id, AGENT_REVIEWER, "✅ Review committed")
    )
    ...
    if branch_name:
        ...
        if pr_number:
            ...
            if success:
                ...
                make_issue_comment(
                    issue_number,
                    format_issue_message(cxc_id, AGENT_REVIEWER, f"📝 Updated PR #{pr_number} with review summary")
                )
            else:
    ...
    make_issue_comment(
        issue_number, format_issue_message(cxc_id, "ops", "✅ Isolated review phase completed")
    )
    
    # Save final state
    state.save("cxc_review_iso")
    
    # Post final state summary to issue
    post_state_to_issue(issue_number, cxc_id, state.data, "📋 Final review state")
