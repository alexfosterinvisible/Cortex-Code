...
# Default agent execution timeout (seconds) - can be overridden by .cxc.yaml
DEFAULT_AGENT_TIMEOUT_SECONDS = int(os.getenv("CXC_AGENT_TIMEOUT", "1800"))  # 30 minutes default
...
def get_model_for_slash_command(
    ...
    """Get the appropriate model for a template request based on CXC state and slash command.

    This function loads the CXC state to determine the model set (base or heavy)
    and returns the appropriate model for the slash command.

    Args:
        request: The template request containing the slash command and cxc_id
    ...
    state = CxcState.load(request.cxc_id)
    if state:
...
def save_prompt(prompt: str, cxc_id: str, agent_name: str = "ops") -> None:
    ...
    config = CxcConfig.load()
    prompt_dir = config.get_agents_dir(cxc_id) / agent_name / "prompts"
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    save_prompt(request.prompt, request.cxc_id, request.agent_name)
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    config = CxcConfig.load()
    output_dir = config.get_agents_dir(request.cxc_id) / request.agent_name
    os.makedirs(output_dir, exist_ok=True)
