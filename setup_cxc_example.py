...
What it does:
    1. Creates/updates pyproject.toml with cxc-framework dependency
...
# ----- CONFIG -----
CXC_FRAMEWORK_DIR = Path(__file__).parent.resolve()
CXC_TEMPLATE_PATH = CXC_FRAMEWORK_DIR / "templates" / "cxc.yaml"
...
def _render_cxc_yaml_template(project_id: str) -> str:
    """Load .cxc.yaml template and inject project_id."""
    if not CXC_TEMPLATE_PATH.exists():
        raise FileNotFoundError(f"Missing .cxc.yaml template: {CXC_TEMPLATE_PATH}")
    template = CXC_TEMPLATE_PATH.read_text()
    if "__PROJECT_ID__" not in template:
        raise ValueError(f"Template missing __PROJECT_ID__ placeholder: {CXC_TEMPLATE_PATH}")
...
'''
    ...
    # Add cxc-framework dependency via uv
    _run(["uv", "add", str(CXC_FRAMEWORK_DIR)], check=False)


def step2_cxc_yaml(target_dir: Path) -> None:
    """Create .cxc.yaml config."""
    print("\n[2/5] Creating .cxc.yaml...")
    cxc_yaml = target_dir / ".cxc.yaml"
    
    if cxc_yaml.exists():
        print(f"  {cxc_yaml} already exists, skipping")
        return
    ...
    content = _render_cxc_yaml_template(project_id)
    cxc_yaml.write_text(content)
    print(f"  Created {cxc_yaml}")
...
def step3_env(target_dir: Path) -> None:
    ...
    lines = ["# CXC Framework Environment Variables\n"]
...
def step3b_commands(target_dir: Path) -> None:
    """Copy CXC command templates to .claude/commands/."""
    print("\n[3b/6] Copying CXC commands to .claude/commands/...")

    commands_src = CXC_FRAMEWORK_DIR / "commands"
    commands_dst = target_dir / ".claude" / "commands"

    if not commands_src.exists():
        print(f"  ⚠️  CXC commands not found at {commands_src}")
        return
...
def step5_verify(target_dir: Path) -> None:
    ...
    if result.returncode == 0 and "Cortex Code" in result.stdout:
        print("  ✓ CXC CLI works!")
    else:
        print("  ✗ CXC CLI failed")
    ...
    result = subprocess.run(
        ["uv", "run", "python", "-c", 
         "from cxc.core.config import CxcConfig; c = CxcConfig.load(); print(f'  ✓ project_id: {c.project_id}')"],
        cwd=target_dir,
        capture_output=True,
        text=True,
    )
...
def main():
    target_dir = Path.cwd()
    print(f"Setting up CXC in: {target_dir}")
    print(f"CXC framework at: {CXC_FRAMEWORK_DIR}")
    ...
    print("  uv run cxc --help")
    print("  uv run cxc plan <issue-number>")
