...
ADW (AI Developer Workflow) is an orchestration framework that automates software development using Claude Code agents in isolated git worktrees. It processes GitHub issues through a complete SDLC pipeline: plan, build, test, review, document, and ship.
...
|:------------------|:--------------------------------|:---------------------|:-------------------------------|:-----------------------------------------|
| `adw plan`        | `wt.plan_iso`                   | `<issue-number>`     | `[adw-id]`                     | Planning phase - creates worktree + plan |
| `adw build`       | `wt.build_iso`                  | `<issue-number> <adw-id>` |                           | Implementation phase                     |
| `adw test`        | `wt.test_iso`                   | `<issue-number> <adw-id>` | `--skip-e2e`             | Testing phase with retry logic           |
| `adw review`      | `wt.review_iso`                 | `<issue-number> <adw-id>` | `--skip-resolution`      | Review against spec with screenshots     |
| `adw document`    | `wt.document_iso`               | `<issue-number> <adw-id>` |                           | Documentation generation                 |
| `adw ship`        | `wt.ship_iso`                   | `<issue-number> <adw-id>` |                           | Approve and merge PR                     |
| `adw patch`       | `wt.patch_iso`                  | `<issue-number>`     | `[adw-id]`                     | Create patch for review issue            |
| `adw sdlc`        | `wt.sdlc_iso`                   | `<issue-number>`     | `[adw-id] --skip-e2e --skip-resolution` | Full pipeline (no ship)   |
| `adw zte`         | `wt.sdlc_zte_iso`               | `<issue-number>`     | `[adw-id] --skip-e2e --skip-resolution` | Full pipeline + auto-ship |
| `adw monitor`     | `triggers.trigger_cron`         |                      |                                | Polling-based trigger                    |
| `adw webhook`     | `triggers.trigger_webhook`      |                      |                                | FastAPI webhook server                   |

### 1.2 CLI Behaviors

| Requirement ID | Description                                                                  |
|:---------------|:-----------------------------------------------------------------------------|
| CLI-01         | `adw --help` shows usage information and exits with code 0                   |
| CLI-02         | `adw` with no command shows help and exits with code 1                       |
...
### 2.1 Configuration File: `.adw.yaml`
...
| `commands`        | `list[string]`  | `["${ADW_FRAMEWORK}/commands"]` | Slash command search paths        |
| `app`             | `dict`          | `{}`                  | App-specific config (backend_dir, etc.)       |

### 2.2 ADWConfig Class Behaviors

| Requirement ID | Description                                                                 |
|:---------------|:----------------------------------------------------------------------------|
| CFG-01         | Loads `.adw.yaml` from project root or walks up directory tree to find it  |
| CFG-02         | Missing `.adw.yaml` uses default values (does not raise)                   |
...
| CFG-06         | `${ADW_FRAMEWORK}` expanded in command paths to framework location         |
...
| CFG-09         | `get_agents_dir(adw_id)` returns `artifacts/{org}/{repo}/{adw_id}`         |
...
### 3.1 ADWState Data Schema

| Field           | Type                | Required | Description                                 |
|:----------------|:--------------------|:---------|:--------------------------------------------|
| `adw_id`        | `string`            | Yes      | 8-character unique workflow ID              |
...
| `all_adws`      | `list[string]`      | No       | List of workflow steps that have run        |

### 3.2 ADWState Class Behaviors

| Requirement ID | Description                                                                   |
|:---------------|:------------------------------------------------------------------------------|
| STATE-01       | `ADWState("")` raises `ValueError` with message "adw_id is required"          |
...
| STATE-05       | `append_adw_id(id)` adds workflow step to `all_adws` list                     |
| STATE-06       | `append_adw_id()` deduplicates (no duplicate entries)                         |
| STATE-07       | `get_working_directory()` returns `worktree_path` if set, else project root   |
| STATE-08       | `get_state_path()` returns `artifacts/{org}/{repo}/{adw_id}/adw_state.json`   |
...
| STATE-12       | `load(adw_id)` returns None if state file doesn't exist                       |
| STATE-13       | `load(adw_id)` parses valid JSON and returns ADWState instance                |
...
| STATE-18       | `from_stdin()` returns None if JSON lacks `adw_id`                            |
| STATE-19       | `to_stdout()` outputs complete JSON including `all_adws`                      |
...
- `adw-id` (optional): Existing ADW ID to resume

**Outputs:**
- Creates worktree at `artifacts/{org}/{repo}/trees/{adw_id}/`
- Creates plan file at `specs/issue-{N}-adw-{ID}-sdlc_planner-{name}.md`
- Creates/updates pull request
- Persists state to `adw_state.json`
...
| Track run                  | `all_adws` += "adw_plan_iso"                           |
...
**Preconditions:**
- State file must exist for `adw_id`
...
| DOC-05         | Creates doc at `app_docs/feature-{adw_id}-{name}.md`                  |
...
|:----------------------------|:-------------------------------|:-----------------------------|:-------------------------------|
| `create_worktree`           | `adw_id, branch_name, logger`  | `(path, error)`              | Creates isolated worktree      |
| `validate_worktree`         | `adw_id, state`                | `(bool, error)`              | Checks worktree exists + valid |
| `get_ports_for_adw`         | `adw_id`                       | `(backend, frontend)`        | Deterministic port allocation  |
| `is_port_available`         | `port`                         | `bool`                       | Checks if port is free         |
| `find_next_available_ports` | `adw_id`                       | `(backend, frontend)`        | Finds free ports               |
...
| Function                      | Inputs                              | Outputs                   | Description                          |
|:------------------------------|:------------------------------------|:--------------------------|:-------------------------------------|
| `ensure_adw_id`               | `issue_number, adw_id?, logger?`    | `str`                     | Creates or validates ADW ID          |
| `classify_issue`              | `issue, adw_id, logger`             | `(command, error)`        | Determines /feature, /bug, /chore    |
| `generate_branch_name`        | `issue, adw_id, logger`             | `(name, error)`           | Creates branch name                  |
| `classify_and_generate_branch`| `issue, adw_id, logger`             | `(cmd, branch, error)`    | Combined LLM call (2x faster)        |
| `build_plan`                  | `issue, cmd, adw_id, log, dir`      | `AgentPromptResponse`     | Runs /feature, /bug, or /chore       |
| `implement_plan`              | `plan_file, adw_id, log, dir`       | `AgentPromptResponse`     | Runs /implement                      |
| `create_commit`               | `agent, issue, cmd, adw_id, log, cwd` | `(msg, error)`          | Runs /commit                         |
| `find_spec_file`              | `state, logger`                     | `str or None`             | Locates plan/spec file               |
| `format_issue_message`        | `adw_id, agent, msg`                | `str`                     | Formats `{adw_id}_{agent}: {msg}`    |
| `post_artifact_to_issue`      | `issue, adw_id, agent, title, content, path, collapsible` | None | Posts formatted artifact |
| `post_state_to_issue`         | `issue, adw_id, state, title`       | None                      | Posts state JSON to issue            |
...
| `adw_id`        | `string`        | Yes      | Workflow ID for context               |
...
| AGENT-05       | Saves prompt to `artifacts/{adw_id}/ops/prompts/{agent}_{timestamp}.md` |
...
|:--------------|:---------------------------------|:---------------------------------|:--------------------------------------------|
| `/feature`    | `$1=issue_num, $2=adw_id, $3=json` | Plan file path                 | `specs/issue-{N}-adw-{ID}-sdlc_planner-{name}.md` |
| `/bug`        | `$1=issue_num, $2=adw_id, $3=json` | Plan file path                 | Same pattern                                |
| `/chore`      | `$1=issue_num, $2=adw_id, $3=json` | Plan file path                 | Same pattern                                |
| `/patch`      | `$1=adw_id, $2=request, $3=spec`   | Patch file path                | `specs/patch/patch-adw-{ID}-{name}.md`      |
...
| `/generate_branch_name`| Issue JSON + adw_id| Branch name string                               |
| `/classify_and_branch` | `$1=adw_id, $2=json` | JSON: `{"issue_class": "/feature", "branch_name": "..."}` |
...
| `/review`                 | adw_id, spec_file, agent | JSON: ReviewResult                  |
| `/document`               | adw_id, spec_path        | Doc file path                       |
...
```
specs/
├── issue-{N}-adw-{ID}-sdlc_planner-{name}.md    # Feature/bug/chore plans
└── patch/
    └── patch-adw-{ID}-{name}.md                  # Patch plans
```
...
├── assets/                    # Screenshots
│   └── *.png
└── feature-{adw_id}-{name}.md # Feature documentation
```
...
|:----------------------|:--------------------------|:----------|
| `adw_plan_iso`        | `plan_iso`                | base      |
| `adw_sdlc_iso`        | `sdlc_iso`                | base      |
| `adw_sdlc_zte_iso`    | `sdlc_zte_iso`            | base      |
...
**Example:** `feat-issue-42-adw-abc12345-add-user-authentication`
