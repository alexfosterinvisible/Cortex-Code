"""
Feature flags for ADW Framework.
...
☑️ GITHUB_COMMENTS: Already implemented via ADW_DISABLE_GITHUB_COMMENTS
⏳ ARTIFACTS_BACKEND: Control artifact storage

Usage:
    from adw.core.features import Features
...
class Features:
    ...
    Load from .adw.yaml or environment variables.
    ...
    def github_comments(self) -> bool:
        """Check if GitHub comments are enabled (respects ADW_DISABLE_GITHUB_COMMENTS)."""
        env_disabled = os.getenv("ADW_DISABLE_GITHUB_COMMENTS", "").strip().lower()
    ...
    def load(cls, config_data: Optional[Dict[str, Any]] = None) -> "Features":
        """
        Load features from config dict (typically from .adw.yaml).
