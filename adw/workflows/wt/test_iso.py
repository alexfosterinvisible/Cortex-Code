...
Usage:
  uv run python -m adw.workflows.wt.test_iso <issue-number> <adw-id> [--skip-e2e]
...
from adw.core.utils import make_adw_id, setup_logger, parse_json, check_env_vars
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m adw.workflows.wt.test_iso <issue-number> <adw-id> [--skip-e2e]")
