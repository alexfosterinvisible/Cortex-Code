...
Usage:
  uv run python -m adw.workflows.wt.document_iso <issue-number> <adw-id>
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m adw.workflows.wt.document_iso <issue-number> <adw-id>")
    ...
    if state:
        ...
        logger.error(
            "Run adw plan or adw patch first to create the worktree and state"
        )
