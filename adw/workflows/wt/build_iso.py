...
Usage:
  uv run python -m adw.workflows.wt.build_iso <issue-number> <adw-id>
...
This workflow REQUIRES that adw plan or adw patch has been run first
...
from adw.integrations.git_ops import commit_changes, finalize_git_operations, get_current_branch
from adw.integrations.github import fetch_issue, make_issue_comment, get_repo_url, extract_repo_path
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m adw.workflows.wt.build_iso <issue-number> <adw-id>")
        print("\nError: adw-id is required to locate the worktree and plan file")
        print("Run adw plan or adw patch first to create the worktree")
        sys.exit(1)
    ...
    if state:
        ...
        logger.error("Run adw plan first to create the worktree and state")
        print(f"\nError: No state found for ADW ID: {adw_id}")
        print("Run adw plan first to create the worktree and state")
        sys.exit(1)
    ...
    if not valid:
        ...
        logger.error("Run adw plan or adw patch first")
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"❌ Worktree validation failed: {error}\n"
                               "Run adw plan or adw patch first")
        )
        sys.exit(1)
    ...
    if not state.get("branch_name"):
        error_msg = "No branch name in state - run adw plan first"
    ...
    if not state.get("plan_file"):
        error_msg = "No plan file in state - run adw plan first"
        logger.error(error_msg)
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"❌ {error_msg}")
