...
def trigger_adw_workflow(issue_number: int) -> bool:
    """Trigger the ADW plan workflow for a specific issue using the CLI."""
    try:
        print(f"INFO: Triggering ADW plan workflow for issue #{issue_number}")
        
        # Use the adw CLI to run the plan workflow
        cmd = [sys.executable, "-m", "adw.cli", "plan", str(issue_number)]
