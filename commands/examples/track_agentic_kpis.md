...
Update or create the CxC performance tracking tables in [app_docs/agentic_kpis.md]. This command analyzes the current CxC run's metrics and maintains both summary and detailed KPI tables. Think hard about building this, these are key KPIs for the Cortex Code (CxC) system. Use the python commands as suggestions and guides for how to calculate the values. Ultimately, do whatever python calculation you need to do to get the values.

## VARIABLES

state_json: $ARGUMENT
attempts_incrementing_cxcs: [`cxc_plan_iso`, `cxc_patch_iso`]
...
- Parse the provided state_json to extract:
  ...
  - issue_class
  - plan_file path
  - all_cxcs list (contains workflow names run)
...
- Count occurrences of any of the cxcs in the attempts_incrementing_cxcs list in all_cxcs list
- Run: `python -c "all_cxcs = <list>; attempts = sum(1 for w in all_cxcs if any(cxc in w for cxc in attempts_incrementing_cxcs)); print(attempts)"`
...
### 4. Update CxC KPIs Table
- Check if current cxc_id already exists in the table
...
#### Current Streak
- Count consecutive rows from bottom of CxC KPIs table where Attempts ≤ 2
...
#### Total Plan Size
- Sum all plan sizes from CxC KPIs table
...
Performance metrics for the Cortex Code (CxC) system.

## Agentic KPIs

Summary metrics across all CxC runs.
...
Detailed metrics for individual CxC workflow runs.

| Date   | CxC ID | Issue Number | Issue Class | Attempts   | Plan Size (lines) | Diff Size (Added/Removed/Files) | Created   | Updated   |
| ------ | ------ | ------------ | ----------- | ---------- | ----------------- | ------------------------------- | --------- | --------- |
